...
export default function CategoryDialog({ mode, initial, onSave, onClose }: Props) {
  const [name, setName] = useState(initial?.name ?? '')
  const [color, setColor] = useState(initial?.color ?? PRESET_COLORS[0])
  // The dialog stays mounted until the parent's save resolves and closes it, so
  // without this a double-click on Create submitted twice.
  const { inFlight: saving, run: runSave } = useInFlight('save category')
